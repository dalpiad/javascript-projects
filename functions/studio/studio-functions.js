...
function reverseCharacters(str) {
    ...
    reversed = stringToArray.reverse().join('');
    return reversed;
}

let myVariableName = 'abcdef';
...
function reverseAllCharacterTypes (str) {
    if (typeof str == 'string') {
        return reverseCharacters(str);
    } else if (typeof str == 'number') {
        return Number(reverseCharacters(String(str)));
    } 
}
...
function reverseArray(oldArray) {
    ...
    for (let i = 0; i < oldArray.length; i++) {
        reversedArray[i] = reverseAllCharacterTypes(oldArray[i]);
    }
...
function sloganGenerator(str) {
    if (str.length <= 3) {
        return `We put the ${str.slice(3)} in ${str}.`;
...
console.log(sloganGenerator(str));
...
console.log(`The area is ${areaOfRectangle(4, 7)}cm^2.`);